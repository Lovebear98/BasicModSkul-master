...
public class HunterPower extends BasePower implements CloneablePowerInterface {
    public static final String POWER_ID = makeID("HunterPower");
    ...
    public HunterPower(AbstractCreature owner, int amount) {
    ...
    public void onUseCard(AbstractCard card, UseCardAction action) {
        if(card.type == AbstractCard.CardType.ATTACK){
            if(owner instanceof LittleBone){
                ...
                ((LittleBone) (AbstractDungeon.player)).AnimateSkull(AtlasPaths.HunterAnim(), "ATTACK");
            }
    ...
    public int onAttacked(DamageInfo info, int damageAmount) {
        if (info.owner != this.owner && info.type != DamageInfo.DamageType.THORNS && damageAmount - owner.currentBlock > 0) {// 372
            ///Play that Skull's animations
            if(owner instanceof LittleBone){
                ((LittleBone) (AbstractDungeon.player)).AnimateSkull(AtlasPaths.HunterAnim(), "DODGE");
            }
    ...
    public AbstractPower makeCopy() {
        return new HunterPower(owner, amount);
    }
}
