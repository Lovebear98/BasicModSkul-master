...
public class FrostSkullPower extends BasePower implements CloneablePowerInterface {
    public static final String POWER_ID = makeID("FrostSkullPower");
    ...
    public FrostSkullPower(AbstractCreature owner, int amount) {
    ...
    public int onAttacked(DamageInfo info, int damageAmount) {
        if (info.owner != this.owner && info.type != DamageInfo.DamageType.THORNS && damageAmount - owner.currentBlock > 0) {// 372
            ...
            if(owner instanceof LittleBone){
                ((LittleBone) (AbstractDungeon.player)).AnimateSkull(AtlasPaths.FrostSkullAnim(), "DODGE");
            }
...
if(card.type == AbstractCard.CardType.ATTACK){
    if(owner instanceof LittleBone){
        ((LittleBone) (AbstractDungeon.player)).AnimateSkull(AtlasPaths.FrostSkullAnim(), "ATTACK");
    }
}
    ...
    public AbstractPower makeCopy() {
        return new FrostSkullPower(owner, amount);
    }
}
