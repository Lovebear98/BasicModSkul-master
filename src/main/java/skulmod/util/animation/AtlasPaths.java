package skulmod.util.animation;

public class AtlasPaths {
    ///The string that decides our animations.
    // We'll store all skulls in Little Bone, even if they're from the booster,
    // since it makes things simple and the design of this makes that less of a sin.
    //Bless Draco 17(+1)
    public static String AbsoluteAnim(){
        return "Absolute";
    }
    public static String BerserkerAnim(){
        return "Berserker";
    }
    public static String BomberAnim(){
        return "Bomber";
    }
    public static String ChampionAnim(){
        return "Champion";
    }
    public static String ClownAnim(){
        return "Clown";
    }
    public static String DarkPaladinAnim(){
        return "DarkPaladin";
    }
    public static String DavyJonesAnim(){
        return "DavyJones";
    }
    public static String FrostSkullAnim(){
        return "FrostSkull";
    }
    public static String GamblerAnim(){
        return "Gambler";
    }
    public static String GodOfWarAnim(){
        return "GodOfWar";
    }
    public static String GreatWarlockAnim(){
        return "GreatWarlock";
    }
    public static String HunterAnim(){
        return "Hunter";
    }
    public static String LittleBoneAnim(){
        return "LittleBone";
    }
    public static String MageAnim(){
        return "Mage";
    }
    public static String ShieldSkullAnim(){
        return "Shield";
    }
    public static String PettyThiefAnim(){
        return "Thief";
    }
    public static String WarriorAnim(){
        return "Warrior";
    }
    public static String WerewolfAnim(){
        return "Werewolf";
    }

    ///Booster Pack Skulls (5)
    public static String HeroSkullAnim(){
        return "HeroSkull";
    }
    public static String GrimReaperAnim(){
        return "GrimReaper";
    }
    public static String LivingArmorAnim(){return "LivingArmor";}
    public static String Minotaurus(){
        return "Minotaurus";
    }
    public static String UnknownKingAnim(){
        return "UnknownKing";
    }
    public static String KingOfHeroesAnim(){
        return "KingOfHeroes";
    }
    public static String CarleonRecruitAnim(){
        return "CarleonRecruit";
    }
    public static String AlchemistAnim(){return "Alchemist";}
    public static String GraveDiggerAnim(){
        return "GraveDigger";
    }

    public static String SamuraiAnim(){
        return "Samurai";
    }
    public static String GeneAnim(){
        return "Gene";
    }


    ///Skulls built using mechanics from other mods
    public static String RiderAnim(){
        return "Rider";
    }





    public static String GhoulAnim(){
        return "Ghoul";
    }
    public static String DemiGod(){return "DemiGod";}
    public static String Sword(){return "Sword";}
    public static String YakshaAnim(){
        return "Yaksha";
    }
}
