package skulmod.util.othermodskulls.unchained.powers;
...
public class RiderPower extends BasePower implements CloneablePowerInterface {
    public static final String POWER_ID = makeID("RiderPower");
    ...
    public RiderPower(AbstractCreature owner, int amount) {
    ...
    public void onUseCard(AbstractCard card, UseCardAction action) {
        if(card.type == AbstractCard.CardType.ATTACK){
            if(owner instanceof LittleBone){
                    ((LittleBone) (AbstractDungeon.player)).AnimateSkull(AtlasPaths.RiderAnim(), "ATTACK");
            }
        }
    ...
    public int onAttacked(DamageInfo info, int damageAmount) {
        if (info.owner != this.owner && info.type != DamageInfo.DamageType.THORNS && damageAmount - owner.currentBlock > 0) {// 372
            ...
            if(owner instanceof LittleBone){
                ((LittleBone) (AbstractDungeon.player)).AnimateSkull(AtlasPaths.RiderAnim(), "DODGE");
            }
    ...
    public AbstractPower makeCopy() {
        return new RiderPower(owner, amount);
    }
