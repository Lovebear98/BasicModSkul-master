...
public class SpeedAltar extends BaseRelic implements ClickableRelic {
    ...
    private static int SpeedAltarGain = 1;

    private static int AltarCap = AlarMaxUses * SpeedAltarGain;
    ...
    public void atBattleStart() {
        super.atBattleStart();
if(this.counter > 0){
    addToBot(new ApplyPowerAction(AbstractDungeon.player, AbstractDungeon.player, new PlatedArmorPower(AbstractDungeon.player, this.counter)));
}
    }
    ...
    return DESCRIPTIONS[0]+this.BoneFragmentCost + DESCRIPTIONS[1] + SpeedAltarGain + DESCRIPTIONS[2] + AltarCap + DESCRIPTIONS [3];
