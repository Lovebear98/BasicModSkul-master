...
public class LittleBonePower extends BasePower implements CloneablePowerInterface {
    public static final String POWER_ID = makeID("LittleBonePower");
    ...
    public LittleBonePower(AbstractCreature owner, int amount) {
    ...
    public float atDamageFinalReceive(float damage, DamageInfo.DamageType damageType) {
        if(damageType == DamageInfo.DamageType.NORMAL){
            return super.atDamageReceive(damage-1, damageType);
        }
    ...
    public int onAttacked(DamageInfo info, int damageAmount) {
        if (info.owner != this.owner && info.type != DamageInfo.DamageType.THORNS && damageAmount - owner.currentBlock > 0) {// 372
            ...
            if(owner instanceof LittleBone){
                ((LittleBone) (AbstractDungeon.player)).AnimateSkull(AtlasPaths.LittleBoneAnim(), "DODGE");
            }
...
if(card.type == AbstractCard.CardType.ATTACK){
    if(owner instanceof LittleBone){
        ((LittleBone) (AbstractDungeon.player)).AnimateSkull(AtlasPaths.LittleBoneAnim(), "ATTACK");
    }
}
    ...
    public AbstractPower makeCopy() {
        return new LittleBonePower(owner, amount);
    }
}
