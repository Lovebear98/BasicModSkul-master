...
public class DarkPaladinSkull extends BaseCard implements SpawnModificationCard {
    ...
    public DarkPaladinSkull(CardInfo cardInfo) {
        super(cardInfo);
    }

    @Override
    public void onObtainCard() {
        GotSkullAction.NewestSkull = this;
        addToTop(new GotSkullAction());
}

    @Override
    public void onChoseThisOption() {
        addToBot(new ApplyPowerAction(AbstractDungeon.player, AbstractDungeon.player, (new DarkPaladinPower(AbstractDungeon.player, -1))));
        if(AbstractDungeon.player instanceof LittleBone){
            ((LittleBone) (AbstractDungeon.player)).AnimateSkull(AtlasPaths.DarkPaladinAnim(), "IDLE");
