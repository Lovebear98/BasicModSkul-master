package skulmod.util.Patches.SwapButton;
...
import static skulmod.SkulMod.swapKey;
...
public class ReloadButton {
    ...
    public boolean ableUse() {
        if(SwapAvailable == TRUE && HasSkull(AbstractDungeon.player)) {
            return true;
        }
        return false;
    }
    ...
    public void render(SpriteBatch sb) {
        if (!Settings.hideEndTurn) {
            ...
            if (Settings.isControllerMode && this.enabled) {
                sb.setColor(Color.WHITE);
                ///Controller BUtton Rendering Here
    ...
    static {
        tutorialStrings = CardCrawlGame.languagePack.getTutorialString(makeID("ReloadButtonUITip"));
        MSG = tutorialStrings.TEXT;
        LABEL = tutorialStrings.LABEL;
        uiStrings = CardCrawlGame.languagePack.getUIString(makeID("ReloadButtonUI")); //CardCrawlGame.languagePack.getUIString("End Turn Button");
        END_TURN_MSG = uiStrings.TEXT[1];
        ENEMY_TURN_MSG = uiStrings.TEXT[2];
        DISABLED_COLOR = new Color(0.7F, 0.7F, 0.7F, 1.0F);
        SHOW_X = 1640.0F * Settings.xScale;
        SHOW_Y = 215.0F * Settings.yScale;
        //Show_Y 45~55 should be immediately under the End Turn button, for Hoshino Compatability
        HIDE_X = SHOW_X + 500.0F * Settings.xScale;
    }

    private static boolean HasSkull(AbstractPlayer player) {
